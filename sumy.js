...
var globLat = 0;
var globLon = 0;
let items = ['🔋 Батарейки 🔋', '📚 Макулатура 📚', '⚙️ Металолом ⚙️', '🔴 Пластиковые крышки 🔴', '🍾 Стекляные бутылки 🍾'];
...
bot.start((ctx) => {
    ...
    ctx.reply("Для начала, отправьте Вашу текущую гео-позицию.")
    return ctx.reply("Напишите /info для доп получения информации") ;
})
bot.on('location', (ctx) => {
    ...
    bot.telegram.sendMessage(ctx.chat.id,'Выберите отходы', {
        reply_markup:{
            keyboard:[
                ['🔋 Батарейки 🔋'],
                ['📚 Макулатура 📚'],
